"""live/select_strategy.py:互動式選策略 + 寫回 live_execution_config.json
——只測試「寫回 JSON」這個有真實副作用的邏輯
...
class TestUpdateStrategyPathInConfig:
    def test_creates_config_from_example_when_missing(self, tmp_path):
        example = tmp_path / "live_execution_config.example.json"
        example.write_text(
            json.dumps(
                {
        ...
        config_path = tmp_path / "live_execution_config.json"
    ...
    def test_updates_existing_config_preserving_other_fields(self, tmp_path):
        example = tmp_path / "live_execution_config.example.json"
        example.write_text(json.dumps({"strategy_path": "strategies/weekend_mean_reversion.yaml", "dry_run": True}))
        config_path = tmp_path / "live_execution_config.json"
    ...
    def test_written_file_ends_with_newline(self, tmp_path):
        example = tmp_path / "live_execution_config.example.json"
        example.write_text(json.dumps({"strategy_path": "strategies/weekend_mean_reversion.yaml"}))
        config_path = tmp_path / "live_execution_config.json"
